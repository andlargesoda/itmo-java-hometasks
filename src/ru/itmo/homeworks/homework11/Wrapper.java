...
public class Wrapper {
    public static void main(String[] args) {
        ...
        // использование конструктор устаревший способ объфвления объекта класса-обертки
        Integer integer01 = 5;
        ...
        // примитивы используются до тех пор пока использовагте классов оберток не станет необходимостью
        ...
        // 2) параметры методов также подлежат упаковке и распаковке
        // 3) автоупаковка и автораспаковка не работают для массивов
        // 4) автоупаковка не работает, если примитив не соответствует типу класса обертки, пример:
        // byte one = 1;
        // Integer i = one; // ошибка
        // 5) автораспаковка работает, если примитив не соответствует типу класса обертки, пример:
        // Byte two = 2;
        // int num2 = two;

        // 6. Полезные методы оберток над примитивами?
        // valueOf(), toString() наиболее часто используемые методы
        ...
        // для сравнения ссылочных типов (класс обертка и т.д.) используем метод equals
        // для сравнения примитивных типов используем оператор сравнения ==

        Integer first = 34;
        Integer second = 34;
        // first, second ссылки на один и тот же объект
        // числа до 128 включительно хранятся в пуле строк
        // пул чисел как и пул строк необходим для экономии памяти
        // оператор == сравнивает ссылки на один и тот же экземпляр, которые находится в пуле чисел
        System.out.println(first == second); // true

        first = 200; // 200 больше 128, имеем первый объект new Integer()
        second = 200; // 200 больше 128, имеем второй объект new Integer()
        // чила больше 128 хранятся как отдельные объекты в
        // оператор == сравнивает ссылки на экземпляры, которые находятся в куче
